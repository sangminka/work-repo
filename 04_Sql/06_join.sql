-- 06_Join.sql
-- 조인(*****)
-- 예제1) 사원번호가(ENO) 7788인 사원의 부서명(DNAME)은 뭘까요?
-- 사원 EMPLOYEE
-- 부서 DEPARTMENT
-- 1) 사원테이블에서 ENO = 7788 인사람의 부서번호(DNO) 를 알아낸뒤 (1번)
SELECT DNO FROM EMPLOYEE
WHERE ENO = 7788;
-- 부서테이블에서 그 부서번호에 해당하는 부서명을 출력하면됨         (2번)
...
-- 2) 조인을 사용함 : 위의 2번조회를 1번 조회로 수정할 수 있음(성능증가)
...
FROM EMPLOYEE EMP
    ,DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO; -- 공통 컬럼으로 연결함 (2개 테이블)
...
--      1-2)부서이름이 SALES인 부서만 출력하세요

SELECT DMP.*,EMP.*
FROM EMPLOYEE EMP
    ,DEPARTMENT DMP
WHERE EMP.DNO = DMP.DNO
...
-- SQL-99 표기법 : DB 상에 100% 호완됨
-- 사용법) SELECT 별칭1.컬럼명, 별칭2. 컬럼명
--        FROM 테이블명1 별칭1 JOIN 테이블명2 별칭2 ON(별칭1.공통걸럼 = 별칭2.공통컬럼)
SELECT ENO,EMP.DNO,DEP.DNO
FROM EMPLOYEE EMP JOIN DEPARTMENT DEP ON(EMP.DNO = DEP.DNO);

-- 레프트(LEFT) 아우터 조인 : 오라클
-- WHREE절에 (+) 없는쪽을 기준으로 명칭을 부여함 : 왼쪽 외부 조인(LEFT OUTER JOIN)
SELECT EMP.ENAME
    ,MAN.ENAME AS MANAGER
FROM EMPLOYEE EMP
    ,EMPLOYEE MAN
...
-- SQL-99 표기법 : DB상에 100% 호완됨
SELECT EMP.ENAME
    ,MAN.ENAME AS MANAGER
FROM EMPLOYEE EMP LEFT OUTER JOIN EMPLOYEE MAN ON(MAN.MANAGER = EMP.MANAGER)
