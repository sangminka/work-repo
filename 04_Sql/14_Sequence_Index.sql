-- 14_Sequence_Index.sql
-- 1. 시퀀스(Sequence) : 호출(실행)할때마다 자동적으로 숫자를 증가시키는 것
-- 목적) 기본키 용도로 사용(중복없음 ,null 없음)
...
-- 시스템 테이블: 시퀀스 확인용 테이블(뷰) : 데이터 사전(DICTIONARY VIEW)
...
-- 예제 3) 사이트에서 조회시 이름검색이(ENAME) 많은데 속도가 느리다고 한다.(EMPLOYEE)
...
-- * 인덱스를 만들어야 하는 기준
-- 1) 테이블에 행의 개수가 많을경우(데이터 건수가 많을경우)
--     :10만건 이상
-- 2) WHERE(조건절)에 해당 컬럼이 많이 등장할 경우
-- 3) 테이블 조인의 공통컬럼으로 사용될 경우

-- 특수인덱스
-- 결합 인덱스 : 컬럼 여러개를 1번에 인덱스로 지정
-- 예)
SELECT * FROM DEPARTMENT
WHERE DNAME = 'SALES'
AND LOC = 'NEW YORK';
