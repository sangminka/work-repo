...
SELECT EMP.ENAME,DEP.DNAME, DEP.LOC
FROM EMPLOYEE EMP JOIN DEPARTMENT DEP ON(EMP.DNO = DEP.DNO)
WHERE EMP.COMMISSION IS NOT NULL;
...
SELECT DEP.DNO AS 부서번호,DEP.DNAME AS 부서명,DEP.LOC AS 부서위치 ,COUNT(*) AS 사원수 ,ROUND(AVG(EMP.SALARY),2) AS 평균급여
FROM EMPLOYEE EMP JOIN DEPARTMENT DEP ON(EMP.DNO = DEP.DNO)
...
-- 사원테이블 : EMPLOYEE
-- 부서테이블 : DEPARTMEN
SELECT DEP.DNAME AS 부서명,EMP.SALARY AS 급여,COUNT(*) AS 사원수
FROM EMPLOYEE EMP JOIN DEPARTMENT DEP ON(EMP.DNO = DEP.DNO)
...
SELECT DEP.DNO,ROUND(AVG(EMP.SALARY),2),MAX(EMP.SALARY),MIN(EMP.SALARY),COUNT(*)
FROM EMPLOYEE EMP JOIN DEPARTMENT DEP ON(EMP.DNO = DEP.DNO)
GROUP BY DEP.DNO;
