...
VALUES(NULL,NULL,NULL,'010-124-5678','SEOUL'); -- 기본키로 인해 에러발생

-- 3) 외래키(참조키) 제약조건: FOREIGN KET(FK)
-- 무결성 제약조건 : 기본키(***) / 외례키(참조키**)
-- 무결성 의미 : 데이터에 결점이 없는것
-- 기본키 : 회원의 ID (중복되면 안됨 + NULL 입력되도 안됨)
-- 참조키 : 부서테이블의 부서번호(DNO : 10 ~ 40), 사원테이블 부서번호(DNO: 10 ~ 30 , 50)
--     부선테이블의 부서번호 값만 사원테이블의 부서번호에 INSERT 할 수 있음(참조 무결성) 
--  자식쪽 컬럼에 참조무결성 제약조건 추가시 참조하는 부모쪽 컬럼은 유일해야함
-- 사용법)CREATE TABLE 테이블명(
...
-- 사용법  CREATE TABLE 테이블명(
--     컬럼명 자료형 CONSTRAINT 제약조건이름 PRIMARY KEY
--   , 컬럼명 자료형 CONSTRAINT 제약조건이름 CHECK(컴럼명_조건식) 
-- );
...
-- CHECK 제약조건 테스트
INSERT INTO EMP_SECOND VALUES(8000,'윤정화',-200); -- 에러 발생(음수)
-- 테이블 삭제
...
WHERE TABLE_NAME IN('EMPLOYEE','DEPERTMENT')
ORDER BY TABLE_NAME;
...
-- 예제1) 사원복사본 테이블에(EMP_COPY) 기본키(PRIMARY) 추가하기 
