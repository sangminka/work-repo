 ...
 * 요약 :
 *      1) stream은 함수형 프로그래밍에서 차용한 클래스로 내부적으로 반복문이 자동 실행됨
 *      2) Stream 은 정렬된 결과를 자료구조(프레임워크) 결과 내보내기가 가능
 *      3) Stream 은 함수(메소드) 체이닝을 이용 (jquery 사용)
 *          예) 함수체이닝: 함수().함수2()...
 *      4) Stream 원본을 조작하지 않고 복사본을 사용함
 *      5) Stream 사용 규칙 : (1) 정의 (2) 가공 (3) 결과 내보내기 : 3단계 규칙
 * <p>
 * ===========================================================
 * DATE            AUTHOR             NOTE
 * —————————————————————————————
