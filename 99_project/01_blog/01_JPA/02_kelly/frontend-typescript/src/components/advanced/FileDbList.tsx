...
function FileDbList() {
    ...
    // 검색어 변수
    const[searchFileTitle,setSearchFileTitle] = useState<string>("");
    // todo 업로드 성공/실패시 화면에 메시지 출력하는 변수
...
//   전체조회 함수
    const retrieveFileDb = () => { 
      // 백엔드 매개변수 전송 : + 현재페이지(page), 1페이지당개수(pageSize)
       FileDbService.getFiles(searchFileTitle, page -1, pageSize)
       .then((response: any)=>{
      // 백엔드 성공시 실행됨
...
//   검색어 수동 바인딩 함수
    const onChangeSearchFileTitle = (e: React.ChangeEvent<HTMLInputElement>) => {
        const searchFileTitle = e.target.value;
        setSearchFileTitle(searchFileTitle);
    };
      ...
      <div className="col-md-8">
        ...
        <div className="input-group mb-3">
          <input
            ...
            onChange={onChangeSearchFileTitle}
          />
