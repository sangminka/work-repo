 ...
 *         사용법 : 예 1) @Slf4j 클래스 위에달고
 *                   2) 로깅 하고 싶은 라인에 log.debug(값.toString());
 *         설치 : Logback 프로퍼터 파일 2개 복사, build.gradle 라이브러리 4개 추가
 *         @Service : 클래스 위에 달고, 달린 클래스는 서버가 가동될때 자동으로 객체 생성됨
 *         (IOC)
 *         예) @Repository, @Component , @Bean 등
 *         @Autowired : 위에서 생성된 객체를 가져오길 할때 사용(DI)
 *         @GetMapping : Get 방식으로 오는 조회 요청에 대해 실행되는 어노테이션
 *         ( 주로 db select 문이 실행됨)
