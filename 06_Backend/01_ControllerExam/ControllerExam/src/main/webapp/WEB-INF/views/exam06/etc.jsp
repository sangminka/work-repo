...
<%--      사용법 :  <c:import url="보여줄 페이지"/> --%>
    <c:import url="footer.jsp"/>
<%--    TODO: 6) url로 페이지 이동하기--%>
...
<%--        chars 문자열에 name 값이 있으면 true, 없으면 false--%>
    <h2>contains : ${fn:contains(chars,name )}</h2>

<%--    TODO: 2) char 문자열에 홍길동 값이 있는 index 번호 리턴--%>
...
<%--    TODO: 5) chars에서 " " 로 문자열 자르기 : 리턴값 : 배열--%>
    <h2><c:set var="sChar" value='${fn:split(chars, " " )}'/> </h2>
    <h2>${sChar[0]}</h2>
    <h2>${sChar[1]}</h2>
    <h2>${sChar[2]}</h2>

<%--    TODO: 6) chars 에서 0 인덱스 ~ 6 인덱스 까지 자르기--%>
