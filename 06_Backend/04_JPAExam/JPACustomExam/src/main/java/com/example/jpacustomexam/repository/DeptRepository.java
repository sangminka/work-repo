...
//    개발자 직접 sql 작성 하는 기능(JPQL) : 1) 쿼리메소드 : 함수이름으로 sql문 만들기(낙타표기법)
//                                       2) @Query 쓰는 방법
//    쿼리메소드 예) findAllByOrderByDnoDesc : findAll - 전체 조회
//                                           OrderBy - 정렬(Order by)
//                                           Dno     - 대상 컬럼명
//                                           Desc    - 내림 차순 [Asc : 오름차순]
...
//    TODO: 3) 전체 조회 + 오름차순(dno) : Asc 생략 가능
...
//          단, nativeQuery = true 사용하세요
//          DTO 클래스에 저장하세요(오라클 쿼리 : 인터페이스로 만듬)
