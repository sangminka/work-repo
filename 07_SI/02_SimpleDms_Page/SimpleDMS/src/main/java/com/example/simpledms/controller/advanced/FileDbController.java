...
public class FileDbController {
    ...
    @GetMapping("/fileDb")
    public ResponseEntity<Object> find(@RequestParam(defaultValue = "") String fileTitle, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "3") int size) {
        try {
...
//          전체조회(dname="") + like 검색(dname="S")
            Page<FileDb> fileDbPage = fileDbService.findAllByDnameContaining(fileTitle, pageable);

//          리엑트 전송 : 부서배열, 페이징 정보[자료구조 : Map<키이름, 값>]
            Map<String, Object> response = new HashMap<>();
            response.put("fileDb", fileDbPage.getContent());     // fileDb 배열(리엑트로 전송)
            response.put("currentPage", fileDbPage.getNumber()); // 현재 페이지 번호
            response.put("totalItems", fileDbPage.getTotalElements()); // 총건수 ( 개수 )
            response.put("totalPages", fileDbPage.getTotalPages()); // 총페이지 수
...
//                데이터 없음
    ...
    public ResponseEntity<Object> create(@RequestParam String fileTitle, @RequestParam String fileContent, @RequestParam MultipartFile fileDb // 첨부 파일
    ) {

        try {
            fileDbService.save(null,   // 기본키
                    fileTitle,   // 제목
                    fileContent, // 본문
                    fileDb       // 첨부파일
            );  // db 저장
...
//    예) <img src="url"/> => 이미지 자동 다운로드해서 화면에 이미 표시 됨
//    http://localhost:8000/api/advanced/filDb/{uuid}                    : 현재 함수 url
//    현재 함수 url == 다운로드 url
//    최종 URL 예 : http://localhost:8000/api/advanced/fileDb/xxxxxiiiii  : 서비스
...
//                  HttpHeader.CONTENT_DISPOSITION : 첨부파일 표시
//                  "attachment; filename=\"" + fileDb.getFileName() + "\"" : 첨부파일명 표시
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + fileDb.getFileName() + "\"")
//           todo : body() : 바디 - 실제 이미지 전송(리엑트)
...
//            서버 에러
    ...
    public ResponseEntity<Object> update(
            ...
            @RequestParam MultipartFile fileDb // 첨부 파일
    ) {
